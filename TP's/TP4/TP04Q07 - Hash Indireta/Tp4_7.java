...
class HashIndiretoLista {
	...
	public int pesquisar(String f) {
		int pos = h(f);
		int resp = tabela[pos].pesquisar(f) == true ? pos : -1;
		return resp;
	}

	public void inserirInicio(Filme f) {
		int pos = h(f.getTitulo_original());
		tabela[pos].inserirInicio(f);
	}
...
class Lista {
	...
	public boolean pesquisar(String f) {
		...
		for (Celula i = primeiro.prox; i != null; i = i.prox) {
			if (i.elemento.getTitulo_original().equals(f)) {
				encontrado = true;
				i = ultimo;
			}
			//comparacoes++;
...
class Tp4_7 {
	...
	public static void main(String[] args) {
		try {
			...
			HashIndiretoLista filmes = new HashIndiretoLista(21);
			...
			for (int i = 0; i < numEntrada; i++) {
				filmes.inserirInicio(new Filme("/tmp/filmes/" + entrada[i]));
			}
