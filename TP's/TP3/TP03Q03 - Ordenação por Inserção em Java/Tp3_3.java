...
class Lista {
	...
	public void insercao() {
		for (int inicio = 0; inicio < n; inicio++) {
			...
			for (
				String s = lista[inicio].getData_de_lancamento();
				fim < n && s.equals(lista[fim].getData_de_lancamento());
				fim++
			);
		...
		try {
			for (int i = 1; i < n; i++) {
				...
				while (
					j >= 0 &&
					dtob
						.parse(lista[j].getData_de_lancamento())
						.compareTo(dtob.parse(tmp.getData_de_lancamento())) >
					0
				) {
					movimentacoes++;
					lista[j + 1] = lista[j];
...
class Tp3_3 {
	...
	public static void main(String[] args) {
		try {
			...
			filmes.sort();
			filmes.mostrar();
			FileWriter file = new FileWriter("536013_insercao.txt");
			filmes.setTermino();
