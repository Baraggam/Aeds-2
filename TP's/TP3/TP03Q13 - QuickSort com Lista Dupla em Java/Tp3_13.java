...
class ListaDupla {
	...
	public void inserirInicio(Filme f) {
		CelulaDupla tmp = new CelulaDupla(f);
		tmp.ant = primeiro;
	...
	public void mostrarInverso() {
		int j = tamanho() - 1;
		for (CelulaDupla i = ultimo; i != primeiro; i = i.ant, j--) {
			MyIO.print("[" + j + "] ");
			i.elemento.imprimir();
			MyIO.println("");
		}
	...
	public void insercao() {
		...
		for (int inicio = 0; inicio < n; inicio++) {
			...
			for (
				String s = percorrer(inicio).elemento.getSituacao();
				fim < n && s.equals(percorrer(fim).elemento.getSituacao());
				fim++
			);
			for (int i = inicio + 1; i < fim; i++) {
				Filme tmp = percorrer(i).elemento;
				int j = i - 1;
				while (
					(j >= inicio) &&
					(
						percorrer(j)
							.elemento.getNome()
							.compareTo(tmp.getNome()) >
						0
					)
				) {
					percorrer(j + 1).elemento = percorrer(j).elemento;
					j--;
				}
				percorrer(j + 1).elemento = tmp;
			}
			inicio += fim - inicio - 1;
		}
	}
	...
	private void quicksort(int esq, int dir) {
		int i = esq, j = dir;
		movimentacoes++;
		Filme pivo = percorrer((dir + esq) / 2).elemento;
		while (i <= j) {
			comparacoes++;
			CelulaDupla aux = percorrer(i);
			while (
				aux.elemento.getSituacao().compareTo(pivo.getSituacao()) < 0
			) {
				i++;
				comparacoes++;
				aux = aux.prox;
	...
	private void swap(int i, int j) {
		movimentacoes += 3;
		Filme temp = percorrer(i).elemento;
		percorrer(i).elemento = percorrer(j).elemento;
		percorrer(j).elemento = temp;
	}
	...
	private CelulaDupla percorrer(int x) {
		try {
			int tam = tamanho();
			if (primeiro == ultimo) {
				throw new Exception("Erro ao remover (vazia)!");
...
class Tp3_13 {
	...
	public static void main(String[] args) {
		try {
			ListaDupla filmes = new ListaDupla();
			filmes.setInicio();
			...
			filmes.sort();
			filmes.mostrar();
			FileWriter file = new FileWriter("536013_quicksort2.txt");
			filmes.setTermino();
