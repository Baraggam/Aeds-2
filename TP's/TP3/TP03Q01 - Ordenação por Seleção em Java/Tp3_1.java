...
class Lista {
	...
	public void insercao() {
		for (int inicio = 0; inicio < n; inicio++) {
			...
			for (
				String s = lista[inicio].getTitulo_original();
				fim < n && s.equals(lista[fim].getTitulo_original());
				fim++
			);
	...
	public void sort() {
		for (int i = 0; i < (n - 1); i++) {
			...
			for (int j = (i + 1); j < n; j++) {
				comparacoes++;
				if (
					lista[menor].getTitulo_original()
						.compareTo(lista[j].getTitulo_original()) >
					0
				) {
					menor = j;
				}
...
class Tp3_1 {
	...
	public static void main(String[] args) {
		try {
			...
			filmes.sort();
			filmes.mostrar();
			FileWriter file = new FileWriter("536013_selecao.txt");
			filmes.setTermino();
