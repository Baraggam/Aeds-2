...
void insercao(Lista *l)
{
	for (int inicio = 0; inicio < l->n; inicio++)
	{
		...
		static char s[1000];
		for (
			strcpy(s, getIdioma_original(l->lista[inicio]));
			fim < l->n && strcmp(s, getIdioma_original(l->lista[fim])) == 0;
			fim++)
			;
...
void insercaoPorCor(Lista *l, int n, int cor, int h)
{
	for (int i = (h + cor); i < n; i += h)
	{
		...
		int j = i - h;
		l->comparacoes++;
		while (j >= 0 && strcmp(getIdioma_original(l->lista[j]), getIdioma_original(tmp)) > 0)
		{
			l->movimentacoes++;
			l->lista[j + h] = l->lista[j];
			j -= h;
			l->comparacoes++;
		}
		l->movimentacoes++;
...
int main()
{
	...
	free(entrada);
	setTermino(filmes);
	FILE *file = fopen("536013_shellsort.txt", "w");
