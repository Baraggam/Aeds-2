...
class Lista {
	...
	public void insercao() {
		for (int inicio = 0; inicio < n; inicio++) {
			...
			for (
				float o = lista[inicio].getOrcamento();
				fim < n && o == lista[fim].getOrcamento();
				fim++
			);
	...
	private void intercalar(int esq, int meio, int dir) {
		...
		Filme tmp = new Filme();
		tmp.setOrcamento(0x7FFFFFFF); //Sentinela
		a1[i] = a2[j] = tmp;
		for (i = j = 0, k = esq; k <= dir; k++) {
			comparacoes++;
			lista[k] =
				(a1[i].getOrcamento() <= a2[j].getOrcamento())
					? a1[i++]
					: a2[j++];
		}
	}
...
class Tp3_9 {
	...
	public static void main(String[] args) {
		try {
			...
			filmes.sort();
			filmes.mostrar();
			FileWriter file = new FileWriter("536013_mergesort.txt");
			filmes.setTermino();
