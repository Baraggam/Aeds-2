...
void insercao(Lista *l)
{
	for (int inicio = 0; inicio < l->n; inicio++)
	{
		...
		static char s[1000];
		for (
			strcpy(s, getTitulo_original(l->lista[inicio]));
			fim < l->n && strcmp(s, getTitulo_original(l->lista[fim])) == 0;
			fim++)
			;
...
void sortRec(Lista *l, int i)
{
	if (i < l->n - 1)
	{
		...
		{
			l->comparacoes++;
			if (strcmp(getTitulo_original(l->lista[menor]), getTitulo_original(l->lista[j])) > 0)
			{
				menor = j;
			}
		}
...
int main()
{
	...
	free(entrada);
	setTermino(filmes);
	FILE *file = fopen("536013_selecaoRecursiva.txt", "w");
