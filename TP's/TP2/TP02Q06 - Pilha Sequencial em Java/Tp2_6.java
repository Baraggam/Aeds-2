...
class Pilha {
	...
	public boolean pesquisar(String f) {
		...
		for (int i = 0; i < n && !encontrado; i++) {
			encontrado = pilha[i].getNome().equals(f);
			comparacoes++;
		}
		return encontrado;
	}
	...
	public boolean pesqBin(String f) {
		...
		while (esq <= dir) {
			meio = (esq + dir) / 2;
			if (pilha[meio].getNome().compareTo(f) == 0) {
				encontrado = true;
				esq = dir + 1;
				comparacoes++;
			} else if (pilha[meio].getNome().compareTo(f) < 0) {
				esq = meio + 1;
				comparacoes += 2;
			} else {
				dir = meio - 1;
...
class Tp2_6 {
	...
	public static void gerenciarPilha(
		Pilha filmes,
		String comandos[],
		int quant
	)
